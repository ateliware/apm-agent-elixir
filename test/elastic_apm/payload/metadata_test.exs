defmodule ElasticAPM.Payload.MetadataTest do
  use ExUnit.Case, async: true

  alias ElasticAPM.Payload.Metadata

  describe "new/1" do
    test "creating a metadata struct with a timestamp" do
      {:ok, app_root} = File.cwd()
      {:ok, timestamp} = NaiveDateTime.new(2018, 1, 1, 1, 5, 3)
      agent_version = Application.spec(:elastic_apm)[:vsn] |> to_string()

      assert %ElasticAPM.Payload.Metadata{
