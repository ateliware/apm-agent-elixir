defmodule ElasticAPM.Plugs.ControllerPlug do
  ...
  def before_send(conn) do
    transaction = %Transaction{name: action_name(conn), uri: "#{conn.request_path}", ip: add_ip_context(conn), type: "Controller" }
    ElasticAPM.Core.Agent.transaction(transaction)
