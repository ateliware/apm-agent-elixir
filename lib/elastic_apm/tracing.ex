defmodule ElasticAPM.Tracing do
  @moduledoc """
  ...
  defmacro transaction(type, name, opts \\ [], do: block) do
    quote do
      TrackedRequest.start_layer(
        ElasticAPM.Tracing.internal_layer_type(unquote(type)),
        unquote(name),
        unquote(opts)
      )
  ...
  Can be be one of `:microseconds | :milliseconds | :seconds`. These come from `t:ElasticAPM.Internal.Duration.unit/0`.
  ...
  ## The duration must have actually occured
    ...
    This function expects that the `ElasticAPM.Internal.Duration` generated by `value` and `units` actually occurs in
