defprotocol ElasticAPM.Transport.Command do
  def message(data)
end

defmodule ElasticAPM.Transport.Command.Register do
  defstruct [:app, :key, :host]
  alias __MODULE__

  defimpl ElasticAPM.Transport.Command, for: __MODULE__ do
...
defmodule ElasticAPM.Transport.Command.StartSpan do
...
defmodule ElasticAPM.Transport.Command.StopSpan do
  @enforce_keys [:timestamp]
...
defmodule ElasticAPM.Transport.Command.StartRequest do
  @enforce_keys [:timestamp]
...
defmodule ElasticAPM.Transport.Command.FinishRequest do
  @enforce_keys [:timestamp]
  defstruct [:timestamp, :request_id]
  alias __MODULE__
...
defmodule ElasticAPM.Transport.Command.TagSpan do
  @enforce_keys [:timestamp]
...
defmodule ElasticAPM.Transport.Command.TagRequest do
  @enforce_keys [:timestamp]
...
defmodule ElasticAPM.Transport.Command.ApplicationEvent do
  @enforce_keys [:timestamp]
...
defmodule ElasticAPM.Transport.Command.CoreAgentVersion do
  defstruct []
  alias __MODULE__
...
defmodule ElasticAPM.Transport.Command.Batch do
  @enforce_keys [:commands]
  defstruct [:commands]
  alias __MODULE__
  alias ElasticAPM.Transport.Command
  alias ElasticAPM.Transport.Layer
  ...
  defimpl ElasticAPM.Transport.Command, for: __MODULE__ do
    def message(%Batch{commands: commands}) do
      %{
        BatchCommand: %{
          commands: Enum.map(commands, &ElasticAPM.Transport.Command.message(&1))
        }
      }
    end
  end
  ...
  defp layer_to_spans(layer, request_id, parent_span_id) do
    span_id = ElasticAPM.Utils.random_string(12)
