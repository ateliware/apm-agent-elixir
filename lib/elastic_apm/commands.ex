defprotocol ElasticAPM.Command do
  def message(data)
end

defmodule ElasticAPM.Command.Register do
...
defmodule ElasticAPM.Command.StartSpan do
...
defmodule ElasticAPM.Command.StopSpan do
  @enforce_keys [:timestamp]
...
defmodule ElasticAPM.Command.StartRequest do
  @enforce_keys [:timestamp]
...
defmodule ElasticAPM.Command.FinishRequest do
  @enforce_keys [:timestamp]
  defstruct [:timestamp, :request_id]
  alias __MODULE__
...
defmodule ElasticAPM.Command.TagSpan do
  @enforce_keys [:timestamp]
...
defmodule ElasticAPM.Command.TagRequest do
  @enforce_keys [:timestamp]
...
defmodule ElasticAPM.Command.CoreAgentVersion do
  defstruct []
  alias __MODULE__
...
defmodule ElasticAPM.Command.Batch do
  @enforce_keys [:commands]
  defstruct [:commands]
  alias __MODULE__
  alias ElasticAPM.Command
  ...
  defimpl ElasticAPM.Command, for: __MODULE__ do
    def message(%Batch{commands: commands}) do
      %{
        BatchCommand: %{
          commands: Enum.map(commands, &ElasticAPM.Command.message(&1))
        }
      }
    end
  end
